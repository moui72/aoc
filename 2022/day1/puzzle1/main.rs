...
fn main() {
    ...
    for (i, elf) in elves.enumerate() {
      ...
      if elf_snacks > 0 {
        ...
        println!("Elf {} has {} snacks with a total of {} calories", i, elf_snacks,elf_calorie_count[i]);
        if elf_calorie_count[i] > max_calories {
          max_calories = elf_calorie_count[i];
          max_cal_elf = i;
        }
        cals_to_elf.entry(elf_calorie_count[i]).and_modify(|e| {e.push(i)}).or_insert(vec![i]);
      } else {
        println!("Elf {} has no snacks", i);
      }
    }

    println!("Elf {} has {} calories, which is the most of any elf", max_cal_elf, max_calories);
    ...
    while top_n > 0 {
      ...
      if num_elves_with_cals <= top_n {
        top_n -= num_elves_with_cals;
        println!("n {}", num_elves_with_cals);
        for _ in 0..num_elves_with_cals {
          top_n_elf_calories += next;
        }
      } else {
        println!("n {}", top_n);
        for _ in 0..top_n {
    ...
    println!("The 3 elves ({}) with the most cals have a total of {}", top_n_elves.join(" "), top_n_elf_calories)
